...
public class CarritoService {
    ...
    private static final BigDecimal VIP_THRESHOLD = BigDecimal.valueOf(10_000);
    ...
    public CarritoResponse crearCarrito(CrearCarritoRequest request, LocalDate fechaSimulada) {
        ...
        // Check VIP expiry: if VIP but no purchases last month → remove VIP
        if (cliente.isEsVip()) {
    ...
    /**
     * Deletes (destroys) a cart that has not been completed.
     */
    @Transactional
    public void eliminarCarrito(Long carritoId) {
        Carrito carrito = findCarritoAbierto(carritoId);
        carrito.setState(EstadoCarrito.DESTRUIDO);
    ...
    private void evaluarPromocionVip(Carrito carrito) {
        ...
        if (totalMes.compareTo(VIP_THRESHOLD) > 0 && !cliente.isEsVip()) {
